		...
		$.getJSON('comments.php', 's=d&p=shoutBox&a=0'+str_last_id,
			function(j)
			{
				if(!j.e)
				{
					...
					{
						var new_post = '<li id="'+c.id+'" class="comment" style="display:none;"><a href="user-'+c.n+'"><img class="user icon" src="'+c.i+'" alt="'+c.u+'" title="'+c.u+'" /></a><div class="comment_msg"><a href="user-'+c.n+'" class="comment_head">'+c.u+'</a><span>'+c.D+'</span><div>'+c.m+'</div></div></li>';
						if(!c.re)
						{
							if(!build)
